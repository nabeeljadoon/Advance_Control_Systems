...
interrupt 0(pin2) and interrupt 1 (pin3) to calculate the pulses. The formula is as belowing:
processVariable = currentPosition * 360/NUMBER_PULSE_REVERLUTION. 
The number of pulses for a revolution is 400x4 = 1600 (taking both rising and falling edges of both channels). 
...
double CYCLE = 0.01;//PID processing interval.
...
/*===================================================================================================*/
void setup() {
  /* pin 7 using timer 4B  0x01 -> 31.374 kHz; 0x02 -> 3.921 kHz; 0x03 -> 409.1 Hz (default)*/
  TCCR4B = (TCCR4B & 0xF8) | 0x02; //change PWM frequency to 3.921Kz 
 ...
 // pinMode(LMS_RIGHT, INPUT);
  ...
  lcd.print("Angular position control");// Print a message to the LCD.
 } 
/*===================================================================================================*/ 
void loop() {
...
knobSetpoint = map(readSetpoint, 0, 1023, 0, 360);
...
if (knobSetpoint < 10) {lcd.print("000"); lcd.print(knobSetpoint); lcd.print("de");}
else if ((knobSetpoint > 9) && (knobSetpoint < 100)) {lcd.print("00"); lcd.print(knobSetpoint); lcd.print("de");}
else if ((knobSetpoint > 99) && (knobSetpoint < 1000)) {lcd.print("0"); lcd.print(knobSetpoint); lcd.print("de");}
else {lcd.print(knobSetpoint); lcd.print("de");}
lastLCDTime = millis();}
...
processVariable = ((double)currentPosition * 360)/1600 ; //1600 pulses/rev   
...
if (manMode == true) {
 ...
 if (autoHome == HIGH && homeSet == LOW){digitalWrite(LED_AUTOHOME, flashBit);
  digitalWrite(DIR_LEFT, HIGH);digitalWrite(DIR_RIGHT, LOW);analogWrite(ENA, controlVariable);}
